...
def build_query():
    ...
    if advanced_conditions_str:
        try:
            ...
            # 如果 advanced_conditions 参数解析出错，打印错误日志
            current_app.logger.error(f"解析 advanced_conditions 错误: {e}")

    if filters:
        query = query.filter(and_(*filters))

    # 当无任何过滤条件时，默认只返回前10条数据
    if not any([education_id, school, data_year, grade, class_name, name, gender, id_card]) and not advanced_conditions_str:
