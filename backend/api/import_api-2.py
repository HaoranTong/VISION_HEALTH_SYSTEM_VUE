...
def import_students():
    # current_app.logger.debug("上传文件请求已到达后端")
    # current_app.logger.handlers[0].flush()  # 强制刷新日志
    """
    学生数据导入接口
    ...
    current_app.logger.debug(f"DEBUG: 原始上传文件名-1: {file.filename}")

    filename = secure_filename_unicode(file.filename)  # 只调用一次 secure_filename
    ...
    # 使用时间戳生成新文件名
    new_filename = f"upload_{int(time.time())}.{ext}"
    temp_filepath = os.path.join(TEMP_UPLOAD_DIR, new_filename)
    ...
    try:
        ...
        # 调试用：输出 DataFrame 的行数和列数
        print("【调试】DataFrame 形状:", df.shape)

        # 新增：检查读取后的 DataFrame
        current_app.logger.debug(f"DEBUG: 文件已成功加载，DataFrame 行数：{len(df)}")
        current_app.logger.debug(f"DEBUG: DataFrame 前五行数据：\n{df.head()}")

        df.columns = df.columns.str.strip()
        current_app.logger.debug(f"DEBUG: DataFrame 列名： {list(df.columns)}")
        current_app.logger.debug(
            f"开始处理 DataFrame，共 {len(df)} 行；表头列数：{len(df.columns)}")
...
# ============== 这里结束插入代码 ==============
        ...
        for index, row in df.iterrows():
            current_app.logger.debug(
                f"处理第 {index+2} 行，数据列数：{len(row)}；表头列数：{len(df.columns)}")

            # 调试用：检查当前行数据的长度是否与 DataFrame 列数一致
            if len(row) != len(df.columns):
                current_app.logger.debug(
                    f"第 {index+2} 行数据列数异常：{len(row)} vs {len(df.columns)}")

                print(
                    f"【调试】第{index+2}行数据长度异常：实际{len(row)}列，预期{len(df.columns)}列")

                current_app.logger.debug(
                    f"处理第{index+2}行数据: {row}")  # 新增：打印当前行数据

                is_valid, errors_req = validate_row(
                    row, required_fields, index)
                if not is_valid:
                ...
                if existing_student and existing_extension:
                    ...
                    if not existing_student:
                        ...
                        current_app.logger.debug("完成所有行数据的处理")
